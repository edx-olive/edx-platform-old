...
class BookmarksAPITests(BookmarkApiEventTestMixin, BookmarksTestsBase):
    ...
    def test_delete_bookmark(self, mock_tracker):
        ...
        self.assertNotEqual(str(self.sequential_1.location), bookmarks_data[0]['usage_id'])
        self.assertNotEqual(str(self.sequential_1.location), bookmarks_data[1]['usage_id'])
    ...
    @patch('openedx.core.djangoapps.bookmarks.api.modulestore',
           return_value=Mock(get_item=Mock(
               return_value=Mock(get_children=Mock(
                   return_value=[Mock(get_children=Mock(
                       return_value=[Mock()]))]))))
           )
    @patch('openedx.core.djangoapps.bookmarks.api.tracker.emit')
    def test_delete_bookmarks_with_chapter_block_type(self, mock_tracker, mocked_modulestore):
        mocked_modulestore().get_item().get_children()[0].get_children()[0].location = self.chapter_2.location
