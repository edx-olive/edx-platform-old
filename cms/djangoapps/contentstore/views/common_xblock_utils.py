...
# get common section updated value

def get_commonsection(request):
    filename = BASE_DIR + '/update_common.json'

    with open(filename, 'r') as f:
        data = json.load(f)
    return JsonResponse({'result': data['total'], 'count': data['count']})


# update value in json file
def update_value(count, total=None):
    if total:
        filename = BASE_DIR + '/update_common.json'

        with open(filename, 'r') as f:
            data = json.load(f)
            data['total'] = total  # <--- add `id` value.
...
def updatecommonsection(request):
    ...
    for course in json_data:
        if course['section'] == 'Introduction':
            ...
            try:
                parent_locator = course['parent_value']
