...
import { avatarImg } from '../components/user/UserPanel';
...
export const getManagerTeam = async (
  ...
  const endpointUrl = managerId
    ? MANAGER_TEAM_URL.replace(/%USER_ID%/g, managerId)
    : MANAGER_TEAM_URL;
  ...
  teamList = teamList.map((member) =>
    Object.assign(
      ...
      { profileImage: member.profileImage ? ("data:image/png;base64," + member.profileImage) : avatarImg }
    )
  );
